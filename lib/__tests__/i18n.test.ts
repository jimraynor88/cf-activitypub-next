...
const ES: typeof EN = {
  ...
  announcements_create: "Crear anuncio",
  announcements_create_placeholder: "Escribe un anuncio…",
  announcements_create_submit: "Publicar",
  announcements_delete: "Eliminar",
  announcements_delete_confirm: "¿Eliminar este anuncio permanentemente?",
...
describe("i18n translations", () => {
  it("EN has all required keys with non-empty strings", () => {
  ...
  it("ES translations are not identical to EN (they are actually translated)", () => {
    const keys = Object.keys(EN) as (keyof typeof EN)[];
    let differentCount = 0;
    for (const key of keys) {
      if (EN[key] !== ES[key]) differentCount++;
    }
    expect(differentCount).toBeGreaterThan(keys.length * 0.9);
  });

  it("no translation key contains template syntax without corresponding value", () => {
    const keys = Object.keys(EN) as (keyof typeof EN)[];
    for (const key of keys) {
